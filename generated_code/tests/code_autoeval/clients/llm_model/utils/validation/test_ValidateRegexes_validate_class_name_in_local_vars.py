...
class ValidateRegexes:
    def validate_class_name_in_local_vars(
        self, class_name: str, local_vars: dict
    ) -> None:
...
@pytest.mark.parametrize(
    "class_name, local_vars",
    [("ParentClass", {"ParentClass": object}), ("AnotherClass", {"AnotherClass": int})],
)
def test_normal_case(class_name, local_vars):
    with patch(
        "code_autoeval.llm_model.utils.validation.validate_regexes.ValidateRegexes"
    ) as mock_validate:
