...
from code_autoeval.llm_model.utils.model.function_attributes import (
    FunctionAttributes, FunctionAttributesFactory)


def create(func: Callable, generated_base_dir: pathlib.Path, class_model: Optional[ForwardRef('ClassDataModel')] = None) -> FunctionAttributes:
    """
    ...
    test_relative_file_path, test_absolute_file_path = (
        FunctionAttributesFactory._construct_file_paths(
            func_name, class_model.module_relative_path if class_model else None, generated_base_dir
        )
    )
...
@patch("code_autoeval.llm_model.utils.model.function_attributes.FunctionAttributesFactory._get_function_name")
def test_create_normal(mock_get_func_name, mock_func, mock_class_model):
    ...
    result = FunctionAttributesFactory.create(mock_func, generated_base_dir, mock_class_model)
...
def test_create_no_class_model(mock_func):
    ...
    with pytest.raises(TypeError):
        FunctionAttributesFactory.create(mock_func, generated_base_dir)    assert result.module_absolute_path == "mock_module"
    assert result.module_relative_path == "mock_module"
    assert result.test_relative_file_path is not None
    assert result.test_absolute_file_path is not None
