...
class ClassDataModelFactory:
    ...
    def _get_coroutine_methods(class_to_process: Any) -> List[str]:
        return [name for name, method in inspect.getmembers(class_to_process) if inspect.iscoroutinefunction(method) or inspect.isasyncgenfunction(method)]
...
async def test_normal_case():
    ...
    with patch("code_autoeval.llm_model.utils.model.class_data_model.ClassDataModelFactory._get_coroutine_methods", return_value=['a_method', 'b_method']):
...
async def test_no_coroutine_methods():
    ...
    with patch("code_autoeval.llm_model.utils.model.class_data_model.ClassDataModelFactory._get_coroutine_methods", return_value=[]):
        result = await ClassDataModelFactory._get_coroutine_methods(mock_class)
        assert result == []
...
async def test_non_class_object():
    ...
    with patch("code_autoeval.llm_model.utils.model.class_data_model.ClassDataModelFactory._get_coroutine_methods", return_value=[]):
        ...
        assert result == []        result = await ClassDataModelFactory._get_coroutine_methods(mock_obj)
        assert result == []