...
@patch("code_autoeval.llm_model.utils.code_cleaning.auto_function_cleaning.AutoFunctionCleaning._remove_unused_arguments")
def test_normal_case(mocked_func, source_code):
    mocked_func.return_value = "def example_function(arg1):\n    result = arg1 + 42\n    print(result)"
...
async def test_async_function():
    ...
    mocked_func.return_value = "async def example_function(arg1):\n    result = arg1 + 42\n    print(result)"
    with patch("code_autoeval.llm_model.utils.code_cleaning.auto_function_cleaning.AutoFunctionCleaning._remove_unused_arguments", mocked_func):
        cleaned_code = await AutoFunctionCleaning("data")._remove_unused_arguments(source_code)
        assert cleaned_code == mocked_func.return_value    mocked_func.return_value = "async def example_function(arg1):\n    result = arg1 + 42\n    print(result)"
    with patch("code_autoeval.llm_model.utils.code_cleaning.auto_function_cleaning.AutoFunctionCleaning._remove_unused_arguments", mocked_func):
        cleaned_code = await AutoFunctionCleaning("data")._remove_unused_arguments(source_code)
        assert cleaned_code == mocked_func.return_value